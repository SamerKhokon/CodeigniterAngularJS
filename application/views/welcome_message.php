...
app.controller('ctrlTags', function($scope,$http,limitToFilter) {
	...
	******************************************************************
	var albums_url = "http://localhost/CodeigniterAngularJS/get-all-albums/";
	...
	*******************************************************************/
	...
	$scope.graph_data = [];
	var graph_data_url = "http://localhost/CodeigniterAngularJS/graph-data/";
	$http.get(graph_data_url).success(function(dt){
		$scope.graph_data = dt;
	});	
	...
	$scope.highchartsNG = {
		...
		series: [{
			data:  $scope.graph_data //[10, 15, 12, 8, 7]
		}],
		title: {
			text: 'Bar Chart'
		},
	...
	$scope.uploadFile = function uploadFile(files) {
			...
			$http.post("http://localhost/CodeigniterAngularJS/upang/", fd, {
